...
class CRUDLGRecord(CRUDBase):
    ...
    # MODIFIED RETURN SIGNATURE to include generated_instruction_html
    async def extend_lg(self, db: Session, lg_record_id: int, new_expiry_date: date, user_id: int) -> Tuple[models.LGRecord, int, str]:
        ...
        if db_lg_record.lg_status_id != models.LgStatusEnum.VALID.value: # Corrected to models.LgStatusEnum
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        try:
            ...
            # This payload should ONLY contain fields that are part of the LGInstructionCreate Pydantic schema.
            instruction_create_payload_for_schema = {
                "lg_record_id": db_lg_record.id,
                "instruction_type": "EXTENSION", # Use the full string for the DB column
                "template_id": instruction_template.id,
                "status": "Instruction Issued",
                "details": instruction_details, # Pass the template data as details
                "maker_user_id": user_id,
                "serial_number": None, # Explicitly set to None, will be generated by crud_lg_instruction.create
            }

            # Determine instruction type code and sub-instruction code for serial generation
            # These are passed separately to _create_instruction_with_retry
            instruction_type_code_enum = InstructionTypeCode.EXT
            sub_instruction_code_enum = SubInstructionCode.ORIGINAL

            # Create a dictionary for parameters specifically for serial generation
            serial_generation_params = {
                'lg_record_id': db_lg_record.id, # Need this for the helper's internal LG fetch
                'instruction_type_code': instruction_type_code_enum,
                'sub_instruction_code': sub_instruction_code_enum,
                'user_id': user_id # Need this for the helper's internal LG fetch (maker_user_id)
            }

            # Call the helper method. The retry logic is now INSIDE crud_lg_instruction.create
            instruction_id = await self._create_instruction_with_retry(
                db, # Pass the original session
                instruction_create_payload=instruction_create_payload_for_schema, # Pass the Pydantic payload
                **serial_generation_params # Unpack this dictionary into keyword arguments
            )
            
            # Fetch the newly created instruction to get its details (including serial_number)
            ...
            # Now generate PDF using the confirmed serial_number from the created instruction
            ...
            # Update the generated_content_path on the instruction after PDF is generated
            generated_content_path = f"gs://your-gcs-bucket/generated_instructions/{filename_for_pdf}.pdf" # Placeholder path
            
            # Update the instruction in the *original* session 'db'
            db_lg_instruction_in_current_session = db.merge(db_lg_instruction) # Merge the object into the current session
            ...
            db.flush() # Flush to save the path

            # In a real system, you would upload generated_pdf_bytes to this path
            # await self.crud_lg_document_instance.upload_to_gcs_from_bytes(generated_pdf_bytes, generated_content_path)

            db.refresh(updated_lg_record) # Refresh LG record to ensure its instructions relationship is updated
            db.refresh(db_lg_instruction_in_current_session) # Refresh instruction to ensure all fields are loaded

        except Exception as e: # Catch all exceptions here, as IntegrityError is handled by crud_lg_instruction.create's retry
        ...
        if not notification_template:
            ...
            template_data = {
                ...
                "instruction_serial": db_lg_instruction.serial_number, # Use the serial from the created instruction
            ...
            email_sent_successfully = await send_email(
                ...
                sender_name=customer.name # Use customer name as sender
            )
        ...
        # MODIFIED RETURN: Now includes the raw HTML content
    ...
    async def release_lg(self, db: Session, lg_record: models.LGRecord, user_id: int, approval_request_id: Optional[int]) -> Tuple[models.LGRecord, int]:
        ...
        if lg_record.lg_status_id in [models.LgStatusEnum.RELEASED.value, models.LgStatusEnum.LIQUIDATED.value, models.LgStatusEnum.EXPIRED.value]: # Corrected to models.LgStatusEnum
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        released_status = db.query(models.LgStatus).filter(models.LgStatus.id == models.LgStatusEnum.RELEASED.value).first() # Corrected to models.LgStatusEnum
        ...
        try:
            ...
            instruction_create_payload_for_schema = {
                ...
                "instruction_type": "RELEASE", # Use the full string for the DB column
                "template_id": instruction_template.id,
                "status": "Instruction Issued",
            ...
            db.refresh(lg_record) # Refresh LG record to ensure its instructions relationship is updated
            db.refresh(db_lg_instruction_in_current_session) # Refresh instruction to ensure all fields are loaded
        ...
        if approval_request_id is None: # Only send this notification if it was a direct action, not through approval
            ...
            if not notification_template:
                ...
                template_data = {
                    ...
                    "instruction_serial": db_lg_instruction.serial_number, # Use serial_number_str here
                    "internal_owner_email": lg_record.internal_owner_contact.email,
    ...
    async def liquidate_lg(self, db: Session, lg_record: LGRecord, liquidation_type: str, new_amount: Optional[float], user_id: int, approval_request_id: Optional[int]) -> Tuple[models.LGRecord, int]:
        ...
        liquidated_status = db.query(models.LgStatus).filter(models.LgStatus.id == models.LgStatusEnum.LIQUIDATED.value).first() # Corrected to models.LgStatusEnum
        ...
        try:
            ...
            instruction_create_payload_for_schema = {
                "lg_record_id": lg_record.id,
                "instruction_type": "LIQUIDATION", # Use the full string for the DB column
                "template_id": instruction_template.id,
                "status": "Instruction Issued",
                "details": instruction_details,
                "maker_user_id": instruction_maker_user_id,
    ...
    async def decrease_lg_amount(self, db: Session, lg_record: LGRecord, decrease_amount: float, user_id: int, approval_request_id: Optional[int]) -> Tuple[models.LGRecord, int]:
        ...
        try:
            ...
            try:
                ...
                instruction_create_payload_for_schema = {
                    "lg_record_id": lg_record.id,
                    "instruction_type": ACTION_TYPE_LG_DECREASE_AMOUNT, # Use the full string for the DB column
                    "template_id": instruction_template.id,
                    "status": "Instruction Issued",
                    "details": instruction_details,
                    "maker_user_id": instruction_maker_user_id,
                ...
                # Now generate PDF using the confirmed serial_number from the created instruction
                filename_for_pdf = f"{db_lg_instruction.serial_number}" # FIX: Use only instruction serial for filename
                generated_pdf_bytes = await generate_pdf_from_html(generated_instruction_html, filename_for_pdf)
                
                # Update the generated_content_path on the instruction after PDF is generated
                # Ensure the path reflects the new filename
                generated_content_path = f"gs://{GCS_BUCKET_NAME}/generated_instructions/{filename_for_pdf}" # FIX: Use the filename_for_pdf
    ...
    async def activate_non_operative_lg(self, db: Session, lg_record: LGRecord, payment_details: LGActivateNonOperativeRequest, user_id: int, customer_id: int, approval_request_id: Optional[int]) -> Tuple[models.LGRecord, int]:
        ...
        if lg_record.lg_type_id != models.LgTypeEnum.ADVANCE_PAYMENT_GUARANTEE.value: # Corrected to models.LgTypeEnum
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Only 'Advance Payment Guarantee' LG types can be activated via this process."
            )

        if lg_record.lg_operational_status_id != models.LgOperationalStatusEnum.NON_OPERATIVE.value: # Corrected to models.LgOperationalStatusEnum
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        valid_status = db.query(models.LgStatus).filter(models.LgStatus.id == models.LgStatusEnum.VALID.value).first() # Corrected to models.LgStatusEnum
        if not valid_status:
        ...
        instruction_details = {
            ...
            "original_lg_status_id": models.LgOperationalStatusEnum.NON_OPERATIVE.value, # Corrected to models.LgOperationalStatusEnum
            "new_lg_status_id": models.LgOperationalStatusEnum.OPERATIVE.value, # Corrected to models.LgOperationalStatusEnum
            "lg_type_id": models.LgTypeEnum.ADVANCE_PAYMENT_GUARANTEE.value # Corrected to models.LgTypeEnum
        }
        instruction_details["lg_amount_formatted"] = f"{lg_record.lg_currency.symbol} {float(lg_record.lg_amount):,.2f}"
        ...
        try:
            ...
            instruction_create_payload_for_schema = {
                "lg_record_id": lg_record.id,
                "instruction_type": ACTION_TYPE_LG_ACTIVATE_NON_OPERATIVE, # Use the full string for the DB column
                "template_id": instruction_template.id,
                "status": "Instruction Issued",
                "details": instruction_details,
                "maker_user_id": instruction_maker_user_id,
        ...
        log_action(
            ...
            {"lg_number": lg_record.lg_number, "old_status": models.LgStatusEnum.NON_OPERATIVE.name, "new_status": models.LgStatusEnum.VALID.name, "payment_details": payment_details.model_dump(), "instruction_id": db_lg_instruction.id, "approved_by_user_id": user_id}, # Corrected to models.LgStatusEnum
            lg_record.customer_id, lg_record.id
        )
        ...
        if approval_request_id is None: # Only send this notification if it was a direct action, not through approval
            ...
            if not notification_template:
                ...
                template_data = {
                    ...
                    "payment_method": payment_details.payment_method,
                    "payment_amount": float(payment_details.amount),
                    "payment_currency_code": payment_currency.iso_code, # Ensure payment_currency is not None
                    "payment_reference": payment_details.payment_reference,
                    "payment_issuing_bank_name": payment_bank.name if payment_bank else "N/A",
                    "payment_date": payment_details.payment_date.isoformat(),
                }
