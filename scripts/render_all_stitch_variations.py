"""
GROW BD — STITCH DARK SAAS COMPLETE MULTI-VARIATION VIDEO GENERATOR
Incorporates all design variations from the Stitch project (17195847435772657649):
- Variation A: Hero Split (Left Text + Right Mockup + 3 Bottom Cards)
- Variation B: Mirrored Split (Left Mockup + Right Text + 3 Bottom Cards)
- Variation C: Vertical Editorial (Left Vertical Feature List + Right Large Mockup)
- Variation D: Centered Executive Showcase (Opening / Closing Finale)
"""
...
# Exact Stitch Dark Palette
BG_COLOR = (9, 13, 22)
CARD_BG = (14, 22, 36)
CARD_BORDER = (28, 42, 64)
CYAN = (0, 229, 204)
SKY = (56, 189, 248)
...
STAGES_DATA = [
    # Scene 0: Grand Opening (Centered Showcase)
    {
        "type": "centered",
        "stage_num": "00",
        "stage_name": "OVERVIEW",
    ...
    # Stage 1: Facility Lines (Variation A: Left Info, Right Mockup)
    {
        "type": "split_left",
        "stage_num": "01",
    ...
    # Stage 2: Issuance Request (Variation B: Left Mockup, Right Info - Mirrored)
    {
        "type": "split_right",
        "stage_num": "02",
    ...
    # Stage 3: Communication (Variation C: Vertical Editorial List on Left + Wide Mockup on Right)
    {
        "type": "vertical_list",
        "stage_num": "03",
        ...
        "cards": [
            {"title": "RFQ & Quotation Matrix", "desc": "Receive competitive bank commission rates and fee structures in one place."},
            {"title": "Bank Form Auto-Fill", "desc": "Direct mapping to official bank application forms without duplicate data entry."},
            {"title": "Turnaround Analytics", "desc": "Benchmark bank execution speeds from submission to active issuance."}
        ]
    },
    # Stage 4: Approval (Variation B: Left Mockup, Right Info - Mirrored)
    {
        "type": "split_right",
        "stage_num": "04",
    ...
    # Stage 5: Issuance (Variation A: Left Info, Right Mockup)
    {
        "type": "split_left",
        "stage_num": "05",
    ...
    # Stage 6: Maintenance (Variation B: Left Mockup, Right Info - Mirrored)
    {
        "type": "split_right",
        "stage_num": "06",
    ...
    # Stage 7: Reconciliation (Variation C: Vertical Editorial List on Left + Wide Mockup on Right)
    {
        "type": "vertical_list",
        "stage_num": "07",
        ...
        "cards": [
            {"title": "Bank vs System Match", "desc": "Instantly compares Bank LG Position against System LG Position."},
            {"title": "Variance Detection", "desc": "Identifies amount differences, expiry shifts, and unrecorded items."},
            {"title": "Structured Resolution", "desc": "Structured exception handling to adjust and verify internal treasury books."}
        ]
    },
    # Scene 8: Closing Finale (Centered Showcase)
    {
        "type": "centered",
        "stage_num": "08",
...
def draw_header(draw):
    draw.rounded_rectangle([36, 18, 56, 38], radius=4, fill=CYAN)
...
def draw_subtitle_dock(draw, text):
    ...
    x1 = (WIDTH - sub_w) // 2
    y1 = HEIGHT - 58
    draw.rounded_rectangle([x1, y1, x1 + sub_w, y1 + sub_h], radius=21, fill=(12, 18, 28, 245), outline=(0, 229, 204, 140), width=1)
...
def render_scene_frame(scene, progress=0.0):
    ...
    if stype == "split_left" or stype == "split_right":
        ...
        # Stage Badge
        draw.rounded_rectangle([tx, 72, tx + 88, 96], radius=12, fill=(14, 26, 38), outline=CYAN, width=1)
        ...
        draw.text((tx, 160), " ".join(desc_words[:5]), fill=TEXT_WHITE, font=FONT_HERO_DESC)
        draw.text((tx, 182), " ".join(desc_words[5:11]), fill=TEXT_MUTED, font=FONT_HERO_DESC)
        draw.text((tx, 204), " ".join(desc_words[11:]), fill=TEXT_MUTED, font=FONT_HERO_DESC)
    ...
    elif stype == "vertical_list":
        # Variation C (From Stitch project): Vertical 3-card stack on left + Wide Mockup on right
        tx = 36
        tw = 370
        mx = 436
        mw = 808
        ...
        # 3 Vertical Cards Stacked on Left
        cy = 165
        for c in scene["cards"]:
            draw.rounded_rectangle([tx, cy, tx + tw, cy + 86], radius=8, fill=CARD_BG, outline=CARD_BORDER, width=1)
            draw.ellipse([tx + 12, cy + 12, tx + 38, cy + 38], fill=(0, 229, 204, 25), outline=(0, 229, 204, 90), width=1)
            draw.text((tx + 21, cy + 17), "*", fill=CYAN, font=FONT_TAG)
            draw.text((tx + 48, cy + 14), c["title"], fill=TEXT_WHITE, font=FONT_CARD_TITLE)
            
            words = c["desc"].split(" ")
            draw.text((tx + 12, cy + 46), " ".join(words[:6]), fill=TEXT_MUTED, font=FONT_CARD_DESC)
            if len(words) > 6:
                draw.text((tx + 12, cy + 62), " ".join(words[6:]), fill=TEXT_MUTED, font=FONT_CARD_DESC)
            cy += 96
            
        # Large Wide Mockup Frame on Right
        draw.rounded_rectangle([mx, 72, mx + mw, 72 + mh], radius=8, fill=(13, 19, 29), outline=(0, 229, 204, 90), width=1)
        draw.ellipse([mx + 12, 82, mx + 20, 90], fill=(239, 68, 68))
        draw.ellipse([mx + 26, 82, mx + 34, 90], fill=(245, 158, 11))
        draw.ellipse([mx + 40, 82, mx + 48, 90], fill=(16, 185, 129))
        draw.text((mx + 58, 78), "Grow BD Treasury Engine — Detailed View", fill=TEXT_DIM, font=FONT_CARD_DESC)
        ...
        if os.path.exists(scene["image"]):
            ...
            cropped = screen.crop((
                int(sw * zoom_offset),
                int(sh * (0.05 + zoom_offset)),
                int(sw * (1.0 - zoom_offset)),
                int(sh * (0.95 - zoom_offset))
            )).resize((mw - 4, mh - 30), Image.Resampling.LANCZOS)
            img.paste(cropped, (mx + 2, 98))
        ...
        draw.rounded_rectangle([WIDTH//2 - 50, 72, WIDTH//2 + 50, 96], radius=12, fill=(14, 26, 38), outline=CYAN, width=1)
...
def build_audio_track(scenes):
    ...
    print("Muxing ElevenLabs audio + BGM with FFmpeg...")
    ...
    cmd_mix = [
        ...
        "-i", temp_vo_combined,
        "-i", BGM_PATH,
        "-filter_complex", "[1:a]volume=0.16[bgm];[0:a][bgm]amix=inputs=2:duration=first[aout]",
        "-map", "[aout]",
        "-c:a", "pcm_s16le",
        TEMP_AUDIO_MIXED
    ]
...
def generate_video():
    print("Starting Multi-Variation Video Generation...")
    ...
    # Save Stills for user review
    for scene in STAGES_DATA:
        frame = render_scene_frame(scene, 0.5)
        out_file = os.path.join(SAMPLES_DIR, f"stitch_variation_{scene['stage_num']}.png")
    ...
    print(f"Final Multi-Variation Video Ready: {OUTPUT_VIDEO_PATH}")
