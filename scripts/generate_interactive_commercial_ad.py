"""
GROW BD TREASURY — INTERACTIVE COMMERCIAL AD WITH SYNCHRONIZED FOCAL HIGHLIGHTS
- Real System UI components integrated
- Synchronized visual spotlights, scan lines, pulsing halos, and animated checkmarks
- Pacing locked directly to the voiceover cues
"""
...
TEMP_VIDEO_SILENT = os.path.join(AUDIO_DIR, "temp_interactive_silent.mp4")
TEMP_AUDIO_MIXED = os.path.join(AUDIO_DIR, "temp_interactive_audio.wav")
...
def draw_focus_spotlight(draw, x, y, w, h, color=TURQUOISE, pulse=1.0, label=""):
    # Draws a high-tech glowing focus box with corner brackets and optional pulsing tag
    pad = 4
    draw.rounded_rectangle([x - pad, y - pad, x + w + pad, y + h + pad], radius=6, outline=color, width=2)
    # Corner markers
    c_len = 10
    draw.line([(x - pad, y - pad), (x - pad + c_len, y - pad)], fill=color, width=3)
    draw.line([(x - pad, y - pad), (x - pad, y - pad + c_len)], fill=color, width=3)
    draw.line([(x + w + pad, y - pad), (x + w + pad - c_len, y - pad)], fill=color, width=3)
    draw.line([(x + w + pad, y - pad), (x + w + pad, y - pad + c_len)], fill=color, width=3)
    draw.line([(x - pad, y + h + pad), (x - pad + c_len, y + h + pad)], fill=color, width=3)
    draw.line([(x - pad, y + h + pad), (x - pad, y + h + pad - c_len)], fill=color, width=3)
    draw.line([(x + w + pad, y + h + pad), (x + w + pad - c_len, y + h + pad)], fill=color, width=3)
    draw.line([(x + w + pad, y + h + pad), (x + w + pad, y + h + pad - c_len)], fill=color, width=3)
    
    if label:
        draw.rounded_rectangle([x, y - 22, x + len(label) * 8 + 16, y - 4], radius=4, fill=color)
        draw.text((x + 8, y - 20), ar(label), fill=(255, 255, 255), font=FONT_TAG)
...
# -------------------------------------------------------------
# DYNAMIC SCENE RENDERERS (With Interactive Focal Highlights)
# -------------------------------------------------------------

def render_scene_0(progress):
    # Scene 0: Focal Highlights on Amount, Beneficiary, and Timeline
    img = Image.new("RGB", (WIDTH, HEIGHT), BG_BASE)
    draw = ImageDraw.Draw(img)
    draw_header(draw)
    
    # Left: Real UI Modal Crop
    lx = 60
    ly = 90
    lw = 620
    lh = 490
    ...
    # Interactive Focal Overlay on the UI
    pulse = 0.5 + 0.5 * math.sin(progress * 15)
    if progress < 0.45:
        # Highlight Amount & Beneficiary in UI
        draw_focus_spotlight(draw, lx + 30, ly + 100, 260, 45, color=TURQUOISE, label="محسوبة بدقة")
    elif progress < 0.85:
        # Highlight Timeline Tracker
        draw_focus_spotlight(draw, lx + 30, ly + 210, lw - 60, 180, color=TURQUOISE, label="كل حركة متسجلة")
        # Animate horizontal scanning beam across the timeline
        scan_y = ly + 210 + int(180 * ((progress - 0.45) / 0.40))
        draw.line([(lx + 30, scan_y), (lx + lw - 30, scan_y)], fill=TURQUOISE_GLOW, width=2)

    # Right: Data Verification Cards
    rx = 710
    ry = 90
    rw = 510
    rh = 490
    ...
    for i, (k, v) in enumerate(chips):
        py = cy + i * 65
        is_active = (progress > (0.15 + i * 0.18))
        bg_col = TURQUOISE_BG if is_active else BORDER_SOFT
        border_col = TURQUOISE if is_active else CARD_BORDER
        
        draw.rounded_rectangle([rx + 30, py, rx + rw - 30, py + 55], radius=8, fill=bg_col, outline=border_col, width=1)
...
def render_scene_1(progress):
    # Scene 1: Animated Facility Utilization Pulse + Sequential Bank Highlights
    img = Image.new("RGB", (WIDTH, HEIGHT), BG_BASE)
    draw = ImageDraw.Draw(img)
    draw_header(draw)
    
    # Left: Real Facilities Page UI
    lx = 60
    ly = 90
    lw = 620
    lh = 490
    ...
    # Interactive spotlight on the utilization meter in left UI
    if progress < 0.55:
        draw_focus_spotlight(draw, lx + 28, ly + 240, lw - 56, 120, color=TURQUOISE, label="حساب استهلاك السقف")

    # Right: Bank Allocation Cards
    rx = 710
    ry = 90
    rw = 510
    rh = 490
    ...
    # Active bank highlights based on progress
    active_bank_idx = int(progress * 6) - 1
    
    row_y = ry + 95
    for i, (bname, blimit, bavail, bpct) in enumerate(banks):
        py = row_y + i * 90
        is_highlighted = (i == active_bank_idx and progress >= 0.45)
        bg_col = TURQUOISE_BG if is_highlighted else BORDER_SOFT
        border_col = TURQUOISE if is_highlighted else CARD_BORDER
        
        draw.rounded_rectangle([rx + 30, py, rx + rw - 30, py + 75], radius=10, fill=bg_col, outline=border_col, width=2 if is_highlighted else 1)
        ...
        # Animated fill bar
        cur_pct = bpct * min(1.0, progress * 1.5)
        draw.rounded_rectangle([rx + rw - 185, py + 42, rx + rw - 45, py + 48], radius=3, fill=(226, 232, 240))
        draw.rounded_rectangle([rx + rw - 185, py + 42, rx + rw - 185 + int(140 * cur_pct), py + 48], radius=3, fill=TURQUOISE)
        
    draw_bottom_dock(draw, SCENES_DATA[1]["subtitle"])
    return cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)

def render_scene_2(progress):
    # Scene 2: Scanning Laser Beam + Animated 87% Match & Checkmarks
    img = Image.new("RGB", (WIDTH, HEIGHT), BG_BASE)
    draw = ImageDraw.Draw(img)
    draw_header(draw)
    
    # Left: Real Request Form UI
    lx = 60
    ly = 90
    lw = 620
    lh = 490
    ...
    # Scanning laser animation across request
    if 0.20 <= progress <= 0.65:
        scan_progress = (progress - 0.20) / 0.45
        scan_y = ly + 75 + int(395 * scan_progress)
        draw.line([(lx + 20, scan_y), (lx + lw - 20, scan_y)], fill=TURQUOISE_GLOW, width=3)
        draw_focus_spotlight(draw, lx + 20, scan_y - 20, lw - 40, 40, color=TURQUOISE, label="فحص التطابق والتكرار")

    # Right: Match Breakdown Cards
    rx = 710
    ry = 90
    rw = 510
    rh = 490
    ...
    # 87% Match Badge (Animates and pulses)
    badge_pulse = (progress >= 0.40)
    badge_bg = TURQUOISE_BG if badge_pulse else BORDER_SOFT
    badge_border = TURQUOISE if badge_pulse else CARD_BORDER
    
    draw.rounded_rectangle([rx + 30, ry + 25, rx + 170, ry + 62], radius=8, fill=badge_bg, outline=badge_border, width=2 if badge_pulse else 1)
    ...
    checks = [
        ("المستفيد (Beneficiary)", "تطابق 100% مع العقود السابقة للمشروع", True, 0.45),
        ("بيانات العقد (Contract)", "مسجل ومعتمد مسبقاً في قاعدة البيانات", True, 0.58),
        ("فترة التغطية (Dates)", "متوافقة مع الجدول الزمني للمشروع", True, 0.70),
        ("تداخل القيمة (Amount)", "تنبيه: تداخل جزئي مع دفعة مقدمة سابقة للمراجعة", False, 0.82)
    ]
    
    cy = ry + 95
    for i, (item_h, item_d, is_ok, trigger_prog) in enumerate(checks):
        py = cy + i * 90
        is_triggered = (progress >= trigger_prog)
        bg_col = BORDER_SOFT if is_ok else AMBER_BG
        border_col = TURQUOISE if (is_triggered and is_ok) else (AMBER_WARN if not is_ok and is_triggered else CARD_BORDER)
        icon_col = TURQUOISE if is_ok else AMBER_WARN
        
        draw.rounded_rectangle([rx + 30, py, rx + rw - 30, py + 76], radius=10, fill=bg_col, outline=border_col, width=2 if is_triggered else 1)
...
def render_scene_3(progress):
    # Scene 3: Approved Ripple Stamp + Bank Form Auto-Fill Highlights
    img = Image.new("RGB", (WIDTH, HEIGHT), BG_BASE)
    draw = ImageDraw.Draw(img)
    draw_header(draw)
    
    # Left: Real Approval Center UI
    lx = 60
    ly = 90
    lw = 620
    lh = 490
    ...
    # Focus spotlight on approval steps in UI
    if progress < 0.40:
        draw_focus_spotlight(draw, lx + 28, ly + 140, lw - 56, 120, color=TURQUOISE, label="اعتماد متعدد المستويات")

    # Right: Auto-Filled Bank Form
    rx = 710
    ry = 90
    rw = 510
    rh = 490
    ...
    draw.rounded_rectangle([rx + 30, ry + 22, rx + 145, ry + 50], radius=6, fill=TURQUOISE_BG, outline=TURQUOISE, width=2)
    ...
    form_fields = [
        ("جهة الإصدار", "Banque Misr — الفرع الرئيسي", 0.35),
        ("اسم طالب الإصدار", "Grow BD Engineering SAE", 0.48),
        ("المستفيد الرسمي", "National Projects Authority", 0.60),
        ("مبلغ ونوع الضمان", "Performance LG — EGP 10,000,000", 0.72),
        ("تاريخ الاستحقاق", "16 March 2027 — صيغة موحدة معتمدة", 0.84)
    ]
    
    sy = ry + 75
    for i, (lbl, val, trigger_prog) in enumerate(form_fields):
        py = sy + i * 58
        is_populated = (progress >= trigger_prog)
        bg_col = TURQUOISE_BG if is_populated else BORDER_SOFT
        border_col = TURQUOISE if is_populated else CARD_BORDER
        
        draw.rounded_rectangle([rx + 30, py, rx + rw - 30, py + 48], radius=6, fill=bg_col, outline=border_col, width=1)
        ...
        if is_populated:
            draw_vector_check(draw, rx + rw - 60, py + 18, size=9, color=TURQUOISE, width=2)
            
    # Delivery Tracker (Animated Progress)
    ty = ry + 380
...
def render_scene_4(progress):
    # Scene 4: Synchronized Matching Laser + Locked Rows & Amber Pulse
    img = Image.new("RGB", (WIDTH, HEIGHT), BG_BASE)
    draw = ImageDraw.Draw(img)
    draw_header(draw)
    
    # Left: Real Reconciliation UI
    lx = 60
    ly = 90
    lw = 620
    lh = 490
    ...
    # Scanning laser across matching records
    if progress < 0.60:
        scan_y = ly + 75 + int(395 * (progress / 0.60))
        draw.line([(lx + 20, scan_y), (WIDTH - 80, scan_y)], fill=TURQUOISE_GLOW, width=2)

    # Right: Matching Rows Locked on Same Horizontal Height
    rx = 710
    ry = 90
    rw = 510
    rh = 490
    ...
    matches = [
        ("رقم مرجع الضمان", "Ref: LG-NBE-99410", True, 0.30),
        ("قيمة الضمان", "EGP 10,000,000 = EGP 10,000,000", True, 0.48),
        ("تاريخ الانتهاء", "16 Mar 2027 = 16 Mar 2027", True, 0.65),
        ("طلب تعديل معلق", "تعديل قيمة +1M (قيد المعالجة بالبنك)", False, 0.80)
    ]
    
    row_y = ry + 95
    for i, (lbl, val, is_matched, trigger_prog) in enumerate(matches):
        py = row_y + i * 90
        is_active = (progress >= trigger_prog)
        ...
        draw.rounded_rectangle([rx + 30, py, rx + rw - 30, py + 76], radius=10, fill=bg_col, outline=tag_col if is_active else CARD_BORDER, width=2 if is_active else 1)
...
def render_scene_5(progress):
    # Scene 5: Grand Finale — Synchronized 4-Pillar Rhythm Pulse & Brand Close
    img = Image.new("RGB", (WIDTH, HEIGHT), BG_BASE)
    draw = ImageDraw.Draw(img)
    draw_header(draw)
...
def build_audio_track(scenes):
    ...
    concat_txt_path = os.path.join(AUDIO_DIR, "voiceover_concat_interactive.txt")
    ...
    temp_vo_combined = os.path.join(AUDIO_DIR, "temp_vo_interactive_combined.wav")
...
def generate_video():
    print("Generating Interactive Synchronized Commercial Video...")
    ...
    for idx, scene in enumerate(SCENES_DATA):
        frame = RENDERERS[idx](0.65)
        out_file = os.path.join(SAMPLES_DIR, f"interactive_scene_{scene['id']}.png")
    ...
    print(f"Final Interactive Video Delivered: {OUTPUT_VIDEO_PATH}")
